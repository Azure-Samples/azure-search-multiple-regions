...
@description('Synchronization method: indexer or changefeed. Controls whether CosmosDbTrigger is included in the deployment package.')
...
resource functionCodeDeploy 'Microsoft.Resources/deploymentScripts@2023-08-01' = {
  ...
  properties: {
    ...
    scriptContent: '''
      ...
      $syncMethod = $env:SYNC_METHOD
      $subscriptionId = $env:SUBSCRIPTION_ID
      
      # Explicitly set subscription context — Publish-AzWebApp requires SubscriptionId to be set
...
'@
      ...
      # For Option 2 (change feed), include the CosmosDbTrigger function that pushes
      # Cosmos DB document changes to both search indexes in real-time
      if ($syncMethod -eq 'changefeed') {
        $cosmosDir = New-Item -ItemType Directory -Path "$tempDir\CosmosDbTrigger" -Force

        # function.json — uses the CosmosDBConnection app setting set by the changefeed
        # settings deployment script during Bicep provisioning
        $cosmosFunctionJson = @{
          bindings = @(
            @{
        ...
        Set-Content -Path "$cosmosDir\function.json" -Value $cosmosFunctionJson

        # function.proj — package references for CosmosDB SDK and JSON serialization
        $cosmosProjContent = @'
<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
...
'@
        Set-Content -Path "$cosmosDir\function.proj" -Value $cosmosProjContent

        # run.csx — pushes changed documents to both primary and secondary search indexes
        $cosmosRunCsx = @'
#r "Newtonsoft.Json"
...
public static async Task Run(IReadOnlyList<Document> documents, ILogger log)
{
    if (documents != null && documents.Count > 0)
    {
        ...
        var payload = new { value = searchDocuments };
...
private static async Task UpdateSearchIndex(string searchService, string indexName, string apiKey, StringContent content, ILogger log, string region)
{
    try
    {
        ...
        if (response.IsSuccessStatusCode)
        {
            log.LogInformation($"{region} search index updated successfully");
        }
        else
        {
            var error = await response.Content.ReadAsStringAsync();
            log.LogError($"{region} search index update failed: {error}");
        }
    }
    catch (Exception ex)
    {
        log.LogError($"Error updating {region} search index: {ex.Message}");
    }
}
'@
        Set-Content -Path "$cosmosDir\run.csx" -Value $cosmosRunCsx

        Write-Host "  [OK] CosmosDbTrigger function included (change feed sync)"
      }
